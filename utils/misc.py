...
def init_distributed_mode(cfg):
    if 'WORLD_SIZE' in os.environ and os.environ['WORLD_SIZE'] != '':
        ...
        cfg.world_size = cfg.world_size * local_world_size
        cfg.gpu = cfg.local_rank = int(os.environ['LOCAL_RANK'])
        cfg.rank = cfg.rank * local_world_size + cfg.local_rank
        print('world size: {}, rank: {}, local rank: {}'.format(cfg.world_size, cfg.rank, cfg.local_rank))
        print(json.dumps(dict(os.environ), indent=2))
    elif 'SLURM_PROCID' in os.environ:
        cfg.rank = int(os.environ['SLURM_PROCID'])
        cfg.gpu = cfg.local_rank = int(os.environ['SLURM_LOCALID'])
        cfg.world_size = int(os.environ['SLURM_NPROCS'])

        print('world size: {}, world rank: {}, local rank: {}, device_count: {}'.format(cfg.world_size, cfg.rank, cfg.local_rank, torch.cuda.device_count()))
    else:
    ...
    print("world_size:{} rank:{} local_rank:{}".format(cfg.world_size, cfg.rank, cfg.local_rank))
